...
version = get_version("src", "django_multiinfo", "__init__.py")
...
setup(
    ...
    url='https://github.com/wooyek/django-multiinfo',
    packages=find_packages('src', exclude=["*.tests", "*.tests.*", "tests.*", "tests"]),
    package_dir={'': 'src'},
    py_modules=[splitext(basename(path))[0] for path in glob('src/*.py')],
    entry_points={
        'console_scripts': [
            'django_multiinfo=django_multiinfo.cli:main'
        ]
    },
